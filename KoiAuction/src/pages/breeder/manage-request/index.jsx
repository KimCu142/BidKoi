...
function BreederRequest() {
  ...
  useEffect(() => {
    ...
    if (initialPrice && !isNaN(parseFloat(initialPrice)) && multiplierOption !== "null") {
      const multiplier = parseInt(multiplierOption.replace("x", ""), 10);
      const calculatedPrice = (multiplier * parseFloat(initialPrice)).toLocaleString() + " VNĐ";
      ...
      try {
        ...
        if (accountId) {
          ...
          if (breederData?.name) {
            ...
            console.log("Breeder Name:", breederData.name);
          }
        } else {
  ...
  useEffect(() => {
    // Fetch dữ liệu Koi khi breederId thay đổi
    if (breederId) {
